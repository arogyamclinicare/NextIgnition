...
export default function AnimatedBeamDemo() {
  ...
  const [tooltipData, updateTooltip] = useState<{
    show: boolean;
    description: string;
    position: { x: number; y: number };
  }>({
  ...
  const features = [
    {
      ...
      gradient:
        'bg-white/80 backdrop-blur-md border border-purple-200/50 shadow-lg',
    },
    {
      ...
      gradient:
        'bg-white/80 backdrop-blur-md border border-emerald-200/50 shadow-lg',
    },
    {
      ...
      gradient:
        'bg-white/80 backdrop-blur-md border border-blue-200/50 shadow-lg',
    },
    {
      title: 'Ignisha AI – Startup Assistant',
      description:
        'Idea validation, growth benchmarking, docs generation & instant insights',
      icon: <Bot className='w-4 h-4 sm:w-5 sm:h-5 text-pink-600' />,
      gradient:
        'bg-white/80 backdrop-blur-md border border-pink-200/50 shadow-lg',
    },
    {
      ...
      gradient:
        'bg-white/80 backdrop-blur-md border border-amber-200/50 shadow-lg',
    },
    {
      title: 'Boost Startup Visibility',
      description:
        'Smart profile ranking to get noticed by the right investors',
      icon: <TrendingUp className='w-4 h-4 sm:w-5 sm:h-5 text-cyan-600' />,
      gradient:
        'bg-white/80 backdrop-blur-md border border-cyan-200/50 shadow-lg',
    },
  ];
      ...
      <div className='flex size-full flex-col max-w-[320px] sm:max-w-2xl md:max-w-none lg:max-w-none max-h-[220px] sm:max-h-[240px] md:max-h-[260px] lg:max-h-[280px] items-stretch justify-between gap-3 sm:gap-4 md:gap-5 lg:gap-6 md:relative lg:relative md:w-full lg:w-full md:h-full lg:h-full'>
        <div className='flex flex-row items-center justify-between md:relative lg:relative md:h-full lg:h-full'>
          <RoundedRect
            ...
            onHover={(show, description, event) =>
              handleHover(show, description, event)
            }
